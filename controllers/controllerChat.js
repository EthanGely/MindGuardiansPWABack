...
module.exports = {
    ...
    createRoom: function (req, res) {
        if (req.userId && req.body.roomTitle) {
            chatModel.createRoom(function (err, success) {
                if (err) {
                    console.log(err)
                    return res.sendStatus(500);
                } else {
    ...
    getMessages: function (req, res) {
        if (req.userId && req.body.roomId) {
            chatModel.getMessages(function (err, messages) {
                if (err) {
                    console.log(err)
                    return res.sendStatus(500);
                } else {
    ...
    postMessage: function (req, res) {
        if (req.userId && req.body.roomId && req.body.message) {
            chatModel.postMessage(function (err, result) {
                if (err) {
                    console.log(err)
                    res.sendStatus(500);
                } else {
            ...
            }, req.userId, req.body.roomId, req.body.message);
        } else {
            res.sendStatus(500);
        }
    }
